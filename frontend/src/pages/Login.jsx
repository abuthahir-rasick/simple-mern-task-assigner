  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (loginMode) {
        ...
        setEmail('');
        setPassword('');
        setModal({ show: true, title: 'Success', message: 'User logged in successfully', isError: false });
      } else {
        await dispatch(register({ email, password })).unwrap();
        setEmail('');
        setPassword('');
        setLoginMode(true);
        setModal({ show: true, title: 'Success', message: 'User registered successfully', isError: false });
      }
    } catch (error) {
      
      const errorMessage = typeof error === 'string' 
        ? error 
        : error?.message || 'Unauthorized or invalid credentials. Please try again.';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-600 px-4">
      ...
      >
        ...
        <p
          className="text-center text-sm text-indigo-600 hover:text-indigo-800 mt-4 cursor-pointer transition"
